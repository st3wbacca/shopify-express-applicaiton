...
app.get('/shopify/callback', (req, res) => {
	...
	if (shop && hmac && code) {
		...
		//remove signature and hmac (hash-based message authentication code)
		delete map['signature'];
		delete map['hmac'];

		const message = querystring.stringify(map); //this is our new querystring
		...
		try {
			...
			hashEquals = crypto.timingSafeEqual(generatedHash,
				providedHmac);
		} catch (error) {
			// timingSafeEqual will return an error if the imput buffers are not the same length
			hashEquals = false;
		};
